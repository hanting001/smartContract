...
export class AdminComponent implements OnInit {
    ...
    async setAdmin() {
        ...
        const hbStorateAddress = await this.web3Service.getAddress('hbStorage');
        console.log(hbStorateAddress);
        this.loadingSer.show();
        hbStorageSC.methods.setAdmin(hbStorateAddress).send({ from: from }).on('confirmation', async (confNumber, receipt) => {
            console.log('----');
            this.loadingSer.hide();
        });
    }
